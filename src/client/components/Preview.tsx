...
interface PreviewState {
  ...
  readonly unseenLogsCount: number
}
const Preview: React.SFC<PreviewProps> = ({
  ...
  const iframeRef = useRef<null | { node: { contentWindow: Window } }>(null)
  ...
  useMemo(() => {
    if (registerToSandBox) {
      const [newSlug, unregisterFromSandBox] = registerToSandBox(
        title || 'Default',
        children,
        sourceCode,
      )
  ...
  const renderedChildren =
    typeof children === 'function' ? children({ log }) : children
  ...
  return (
    <div
      className={`Doky__Preview${
        state.height !== 'auto' ? ' Doky__Preview--ready' : ''
      }${className ? ` ${className}` : ''}`}
      onClick={handleClick}
    >
      <div className="Doky__PreviewHeader">
        ...
        <button
          type="button"
          name="Console"
          className="Doky__PreviewButton mr-2"
        >
          {state.unseenLogsCount > 0 && (
            <span className="Doky__PreviewButtonUnseen">
      ...
      <Frame
        ref={iframeRef}
        className="Doky__PreviewFrame"
        head={previewHeadHtml}
        scrolling="no"
