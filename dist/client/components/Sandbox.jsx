...
(function (InspectorView) {
    InspectorView[InspectorView["Console"] = 0] = "Console";
    InspectorView[InspectorView["Source"] = 1] = "Source";
...
const Sandbox = ({ className, open, onOpenStateChange, activeRouteId, pages, currentPage, }) => {
    ...
    const renderedChildren = activeItem
        ? typeof activeItem.children === 'function'
            ...
            : activeItem.children
        : null;
    return (<div className={`DokyModal${open ? ' DokyModal--Open' : ''}${className ? ` ${className}` : ''}`}>
      <div className="Doky__ModalContainer">
            ...
            <div className="DokySandboxSidebar">
              <div className="DokySandboxSidebarHeader">
                <h4 className="DokySandboxSidebarHeader__Title">
                  <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="#ece4ff" stroke="#403294" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="feather feather-package">
                    <path d="M12.89 1.45l8 4A2 2 0 0 1 22 7.24v9.53a2 2 0 0 1-1.11 1.79l-8 4a2 2 0 0 1-1.79 0l-8-4a2 2 0 0 1-1.1-1.8V7.24a2 2 0 0 1 1.11-1.79l8-4a2 2 0 0 1 1.78 0z"/>
                    <polyline points="2.32 6.16 12 11 21.68 6.16"/>
                    <line x1="12" y1="22.76" x2="12" y2="11"/>
                    <line x1="7" y1="3.5" x2="17" y2="8.5"/>
                  </svg>
                  <select name="" className="input DokySandboxSibebarHeader__Select" value={currentPage ? currentPage.id : ''} onChange={handleSelectChange}>
                    {pages.map(page => (<option key={page.id} value={page.id}>
                        {page.meta.title}
                      </option>))}
                  </select>
                </h4>
              ...
              <div className="DokySandboxSidebar__Body">
                {currentPage &&
        currentPage.sandboxRoutes.map((route, i) => (<a key={route.id} href={`#${HASH_PREFIX}${route.id}`} className={`DokySandboxSidebar__Item${(activeRouteId
            ...
            <div className="DokySandboxBody">
              <div className="DokySandboxHeader">
                {activeItem && (<h4 className="DokySandboxHeader__Title">
                    {activeItem.title}
                  </h4>)}
              </div>
              <div className="DokySandboxPreview">
                <Frame className="DokySandboxPreview__Frame" head={getPreviewHead()} scrolling="no">
                  <div>
                    {renderedChildren}
                    <style>{`
                  .frame-root {
              ...
              <div className="DokySandboxInspector">
                <div className="DokySandboxInspector__Header">
                  <button className={`DokySandboxInspector__Tab${state.inspectorView === InspectorView.Console
        ...
        payload: {
            inspectorView: InspectorView.Console,
                  ...
                  <button className={`DokySandboxInspector__Tab${state.inspectorView === InspectorView.Source
        ? ' DokySandboxInspector__Tab--Active'
        : ''}`} onClick={() => dispatch({
        type: ACTION_OVERRIDE,
        payload: {
            inspectorView: InspectorView.Source,
                ...
                <div className="DokySandboxInspector__Body">
                  <div ref={scrollNodeRef} className="DokySandboxInspector__Scroll">
                    {state.inspectorView === InspectorView.Console &&
        state.logs.map((logItem, i) => (<ul key={i} className={`DokySandboxInspector__LogItem${i % 2 ? ' DokySandboxInspector__LogItem--2' : ''}`}>
                          <Inspector name={logItem.name} data={logItem.message}/>
                        </ul>))}
                    {state.inspectorView === InspectorView.Source &&
        activeItem &&
        activeItem.sourceCode && (<div dangerouslySetInnerHTML={{
...
//# sourceMappingURL=Sandbox.jsx.map